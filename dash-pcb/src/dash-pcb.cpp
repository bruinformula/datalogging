...
void read_CAN() {
  ...
  while (can3.read(msg)) {
    ...
    if (msg.id == 0x01F0A000) {
      ...
      */
      tele_data_2B[0][0] = msg.buf[0];
      tele_data_2B[0][1] = msg.buf[1];  // engine_speed
      tele_data_2B[1][0] = msg.buf[2];
      tele_data_2B[1][1] = msg.buf[3];  // engine_load
      tele_data_2B[2][0] = msg.buf[4];
      tele_data_2B[2][1] = msg.buf[5];  // throttle
      ...
      tele_data_2B[4][0] = msg.buf[6];
      tele_data_2B[3][1] = msg.buf[7];  // vbat
    } else if (msg.id == 0x01F0A004) {
      tele_data_2B[3][0] = msg.buf[0];
      tele_data_2B[3][1] = msg.buf[1];              // MAP
  ...
  if (micros() > next_shift_light_frame_micros) {
    ...
    if (RPM < 600) {
      ...
      if (RPM < SL_RPM_MIN) {
        ...
        number_of_lights =
            (RPM - SL_RPM_MIN) * 7 / (SL_RPM_MAX - SL_RPM_MIN) + 1;
      }

      if (number_of_lights > 8) number_of_lights = 8;
